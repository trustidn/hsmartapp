        ...
        <div class="relative">
          ...
          <Transition name="dropdown">
            <div
              ...
              class="absolute right-0 top-full mt-2 min-w-[280px] w-[90vw] max-w-[320px] py-2 bg-white rounded-2xl shadow-lg border border-gray-100 z-30"
            >
              <router-link
                to="/app/subscription"
                class="flex items-center gap-4 px-5 py-4 text-base text-gray-700 hover:bg-gray-50 active:bg-gray-100 touch-manipulation min-h-[52px]"
                @click="showSettingsMenu = false"
              >
                <svg class="w-6 h-6 text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z"/></svg>
                Langganan
              </router-link>
              <router-link
                to="/app/settings"
                ...
                @click="showSettingsMenu = false"
              >
                <svg class="w-6 h-6 text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                Pengaturan
              </router-link>
              <button
                type="button"
                class="flex items-center gap-4 w-full px-5 py-4 text-base text-gray-700 hover:bg-gray-50 active:bg-gray-100 text-left disabled:opacity-60 touch-manipulation min-h-[52px]"
                :disabled="!isOnlineVal || syncing"
                @click="manualSync"
              >
                <svg v-if="syncing" class="w-6 h-6 text-gray-500 shrink-0 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                <svg v-else class="w-6 h-6 text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                {{ syncing ? 'Menyalin...' : 'Sinkronkan data offline' }}
              </button>
              <hr class="my-1 border-gray-100" />
              <button
                type="button"
                class="flex items-center gap-4 w-full px-5 py-4 text-base text-red-600 hover:bg-red-50 active:bg-red-100 text-left touch-manipulation min-h-[52px]"
                @click="handleLogout"
              >
